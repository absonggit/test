...
root_file_check () {
    ...
    echo "【文件系统24小时内的文件修改】"
    echo "------------------------------------"
    find / ! -path "/var/lib/filebeat/*" ! -path "/var/run/*" ! -path "/tmp/sess*" ! -path "/usr/local/php/var/log/*" ! -path "/home/wwwlogs/*" ! -path "/home/wwwroot/*" ! -path "/var/log/*" ! -path "/proc/32737" ! -path "/proc/*" ! -path "/sys/*" -type f -mtime 0
}
ip_check
netstat_check
hosts_check
