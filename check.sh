...
items_check () {
    ...
    do
        ...
        printf "%-10s %-30s\n" ${file_count} $path
        echo "24小时内修改过的文件"
        find $path ! -path "$path/data/Runtime/*" ! -path "$path/runtime/*" ! -path "$path/data/runtime/*" ! -path "$path/data/logs/*" -mtime 0 -type f  ! -name "*.png" ! -name "*.log" -print || {
            echo none
        }
        echo "------------------------------------"
    done
...
root_file_check () {
    ...
    echo "【文件系统24小时内的文件修改】"
    echo "------------------------------------"
    find / ! -paht "/proc/26595" ! -path "/proc/19724" ! -path "/proc/32047" ! -path "/proc/32065" ! -path "/proc/32523" ! -path "/proc/32526" ! -path "/run/systemd/*" ! -path "/var/cache/man/*" ! -path "/usr/local/aegis/*" ! -path "/usr/local/share/aliyun-assist/*" ! -path "/var/lib/filebeat/*" ! -path "/var/run/*" ! -path "/tmp/sess*" ! -path "/usr/local/php/var/log/*" ! -path "/home/wwwlogs/*" ! -path "/home/wwwroot/*" ! -path "/var/log/*" ! -path "/proc/32737" ! -path "/proc/663" ! -path "/proc/12100" ! -path "/proc/*" ! -path "/sys/*" -type f -mtime 0
}
ip_check
netstat_check
hosts_check
